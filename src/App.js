...
const ImmersiveLightbox = ({
  ...
  // FIX: 导航保险丝状态，防止在等待加载时，UI完全锁死
  const [isNavigationUnlocked, setIsNavigationUnlocked] = useState(false);

  useEffect(() => {
    setIsHighResLoaded(false);
    setIsNavigationUnlocked(false); // 每次切换图片，重新锁定导航状态

    let loadUnlockTimer;
    let criticalUnlockTimer;
    ...
    if (highResRef.current && highResRef.current.complete) {
      ...
      // 2. 设置临界超时解锁：3秒后强制解锁导航和图片状态
      criticalUnlockTimer = setTimeout(() => {
        setIsHighResLoaded(true); // 强制淡入，哪怕没加载完
        setIsNavigationUnlocked(true); // 强制解锁 UI
        console.warn("CRITICAL LOAD TIMEOUT: Navigation unlocked forcefully.");
      }, 3000);
    }
    ...
    return () => {
      if (loadUnlockTimer) clearTimeout(loadUnlockTimer);
      if (criticalUnlockTimer) clearTimeout(criticalUnlockTimer);
    };
  }, [currentIndex, images]);

  const changeImage = (direction) => {
    // FIX: 强制中断当前加载周期，立即进入下一张状态
    setIsHighResLoaded(false);
    setIsNavigationUnlocked(false);
  ...
  return (
    ...
    >
      ...
      {/* Fixed Arrow Navigation (仅在解锁后可点击) */}
      ...
      {/* Interactive Click Zones (必须在解锁后才能操作) */}
      <div
        className={`absolute inset-y-0 left-0 w-1/2 z-10 cursor-pointer ${
          isNavigationUnlocked ? "" : "pointer-events-none"
        }`}
      ...
      <div className="relative z-0 w-full h-full flex items-center justify-center p-4 pointer-events-none">
        ...
        <img
          ...
          onLoad={() => {
            setIsHighResLoaded(true);
            setIsNavigationUnlocked(true); // 确保图片加载成功后解锁导航
          }}
          onError={() => {
            setIsHighResLoaded(true); // 图片加载失败也应该解锁，以免卡死
            setIsNavigationUnlocked(true);
          }}
        />
      </div>
